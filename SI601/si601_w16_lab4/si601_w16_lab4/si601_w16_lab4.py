...
with sqlite.connect(r'si601_f15_lab4.db') as con: 
  cur = con.cursor()
  ...
  cur.execute("SELECT nation.N_NAME, SUM(customer.C_ACCTBAL) FROM nation JOIN customer ON (nation.N_NATIONKEY=customer.C_NATIONKEY) GROUP BY N_NAME")
  # fetchall() method gets all records. It returns a list of tuples.
  # Each of the tuples represent a row in the table.
  rows = cur.fetchall()
  ...
  cur.execute("SELECT nation.N_NAME, COUNT(*) AS N_COUNT FROM nation JOIN customer ON (nation.N_NATIONKEY=customer.C_NATIONKEY) WHERE customer.C_MKTSEGMENT = 'AUTOMOBILE' GROUP BY N_NAME ORDER BY N_COUNT DESC")
  # fetchall() method gets all records. It returns a list of tuples.
  # Each of the tuples represent a row in the table.
  rows = cur.fetchall()
